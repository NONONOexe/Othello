...
public class Formula {
	private SetPoint aiFPoint;
	private BoardState board;

	public Formula(BoardState board, int limit) {
		this.board = board;
		int eval;// 評価値
		int eval_max = -100;// 最大評価値(最小を-100としている)
		int alpha = -100;
		int beta = 100;
		...
		if (rabit.match()) {
			aiFPoint = rabit.getPoint();
		} else if (tiger.match()) {
			aiFPoint = tiger.getPoint();
		} else if (mouse.match()) {
			aiFPoint = mouse.getPoint();
		} else if (cow.match()) {
			aiFPoint = cow.getPoint();
		} else if (horse.match()) {
			aiFPoint = horse.getPoint();
		} else {
	...
	private int myturn(BoardState board, int limit, int alpha, int beta) {
		...
		if (board.isGameOver() || limit == 0) {
			if (BoardState.MAX_TURNS - board.getTurns() <= 10) {
				// 終了5ターン以内なら、完全読み
				if (BoardState.MAX_TURNS - board.getTurns() <= 5) {
	...
	private int yourturn(BoardState board, int limit, int alpha, int beta) {
		...
		if (board.isGameOver() || limit == 0) {
			if (BoardState.MAX_TURNS - board.getTurns() <= 10) {
				...
				// それ以外はMovablePosを基準に測る
				Value yourvalue = new EvalCommon();
				int y = yourvalue.valueMain(board);
				return y;
			}
		}
